...
| **R-2b** | **LIVE BUG — sleep-schema drift breaks the coaching day (HIGH).** The coaching-day resolver reads `sleep_schedule.bedtime` (`coaching_day.py:57`); the **onboarding text-edit** writer stores `typical_bedtime`/`typical_wake_time` (`onboarding.py:2670-2673`). Empirically verified: `_parse_bedtime(onboarding_shape) → None` → the day silently falls back to calendar midnight for users who set sleep via onboarding (the HealthKit-import path writes `bedtime` and works). | Fix by normalizing the sleep-schedule fact shape at one writer/reader boundary; fold into DayPlan Phase 1 (it owns wake/sleep). Add a regression test with both shapes. |
