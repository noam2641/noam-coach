...
| 62 | Proactive morning briefing and daily menu delivery | Screenshots show the morning experience pushing a long full-day menu when the user expects a concise daily briefing and next action. | `job_morning` can still send a full daily menu proactively even though `menu:morning` is defined as a short briefing. | Make proactive morning delivery short by default and keep the full daily menu behind explicit request or opt-in behavior. | [`tasks/TASK_62_PROACTIVE_MORNING_BRIEFING_AND_DAILY_MENU_DELIVERY.md`](../tasks/TASK_62_PROACTIVE_MORNING_BRIEFING_AND_DAILY_MENU_DELIVERY.md) | Scheduled morning message is concise; full daily menu is explicit/opt-in; no duplicate/conflicting nutrition summaries; tests cover proactive and on-demand morning surfaces. |
| 63 | Plan completion question deduplication and continuation | Screenshots show the allergy/sensitivity question being asked repeatedly after the user answered "אגוזים". | Plan-completion free-text answers can fail to advance or can re-render the same top-level question through overlapping pending/question/classification state. | Enforce a post-answer invariant: after a successful save, the next rendered question must not repeat the same fact unless a structured clarification remains unresolved. | [`tasks/TASK_63_PLAN_COMPLETION_QUESTION_DEDUPLICATION_AND_CONTINUATION.md`](../tasks/TASK_63_PLAN_COMPLETION_QUESTION_DEDUPLICATION_AND_CONTINUATION.md) | Free-text allergy answers record and advance; classification sub-flow runs once; redundant-question complaints inspect stored facts; tests cover button, free-text, and repeated-question paths. |
| 64 | Multi-fact free-text profile update during active flows | Screenshots show compact correction text such as "גובה 174 אימונים מזוהים 4" during profile/Health confirmation. | Free-text handling is mostly single-question/single-intent, so multiple facts in one message can be ignored, misrouted, or fail to advance the active flow. | Add shared multi-fact extraction and validated saving for profile/Health/plan-completion flows while preserving active-flow continuation. | [`tasks/TASK_64_MULTI_FACT_FREE_TEXT_PROFILE_UPDATE_DURING_ACTIVE_FLOWS.md`](../tasks/TASK_64_MULTI_FACT_FREE_TEXT_PROFILE_UPDATE_DURING_ACTIVE_FLOWS.md) | Multi-fact Hebrew updates save the correct fact keys, confirm updated fields, ask only targeted clarifications, and continue the active flow. |
| 65 | Focused "What should I eat now?" response | Screenshots show "מה לאכול עכשיו" returning daily status, future planning, after-meal calculations, and long rationale instead of a compact immediate answer. | The first next-meal renderer still combines immediate recommendation, daily status, workout status, timeline, and explanation. | Split first-screen next-meal rendering from detail/status surfaces so the default response gives one immediate eating action. | [`tasks/TASK_65_FOCUSED_WHAT_TO_EAT_NOW_RESPONSE.md`](../tasks/TASK_65_FOCUSED_WHAT_TO_EAT_NOW_RESPONSE.md) | First screen is compact; one immediate recommendation appears by default; timeline/status/calculation details move behind secondary actions; safety and workout timing remain enforced. |
