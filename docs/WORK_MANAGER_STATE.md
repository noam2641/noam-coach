...
| Active task | **A10 (degraded-safety readiness) — validated, merge held for owner.** Branch `feat/a10-degraded-plan`, worktree `wt-a10`, PR #74. Invariant now enforced on EVERY path: degraded proposal → conservative behaviour (`SAFETY_UNKNOWN` on the built profile) → disclosure → explicit confirmation → activation through A9 only. **Two errors corrected during implementation.** (1) A fifth enforcement point existed — `training_limitations` is required by the `"workout"` profile as well as `"safety"`, so the degraded path was unreachable while appearing implemented. (2) A first attempt deferred `build_weekly_plan`'s retirement to A11b; the owner rejected that and §3.7 already said the dependency runs A10 → A11b. `build_weekly_plan` now proposes a candidate through the canonical pipeline and writes no governed fact; `planning.activate_plan` is the sole writer; `onboarding.py` is out of `_ALLOWED_FACT_WRITER_FILES`. Confirmation reuses the existing `planv2:select:` tap. 15 deliberate-breakage mutations caught in total. Merge order unchanged: A10 → A11b → A12. |
...
| Approved work outstanding | **Track A**: A8, A10, A11b, A12, **A13** (A1–A6, A9 and A11a done, all merged with reachability proven. **W1-44 is closed by A9.** A7 is DONE (was held serial behind A9: `safety` readiness requires `training_limitations`, whose staleness semantics A7 changes, which changes when A9's realignment is permitted to succeed — A9's boundary tests re-run green with A7 present). **Track B** (WAVE-1 survivors): W1-11, W1-13, W1-14, W1-15, W1-16, W1-18, W1-20, W1-21, W1-22, W1-23. |
