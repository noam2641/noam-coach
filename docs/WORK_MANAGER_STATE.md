...
| Field | Value |
|---|---|
| Last updated | 2026-08-02 (A11b merged: PR #75, merge `28f98f3`; docs correction `a16f1eb`) |
| Last verified `origin/develop` | `a16f1eb` — A11b implementation `37c5bc3` proven reachable (exit 0); both suite halves exit 0 (2670 + 454 collected); ruff and compileall exit 0; both CI contexts green on `37c5bc3` |
| GitHub default branch | `develop` |
| Active phase | **Personalized-workout programme, Track A — wave 2.** A9, A7, A10 and A11b merged with reachability proven (PRs #70–#75). Zero open PRs, single worktree. *(The 2026-07-27 WAVE-1 audit wave is closed; its survivors are listed under Track B below.)* |
| Active task | **A11b COMPLETE** (PR #75, `37c5bc3`, merge `28f98f3`; `git merge-base --is-ancestor 37c5bc3 origin/develop` exit 0). Slot identity is `<stable_session_key>:<slot_key>` -- both components declared, neither derived from weekday, array position, current exercise id or a traversal counter. Two schemes rejected on measurement: `<code>:<ordinal>` gave 10 duplicate ids of 20 in a 6-day plan; `<code>#<Nth>` made two sessions sharing a code SWAP identities under reordering. All ten split producers declare keys; no positional fallback; a split/key mismatch fails closed. Both `.index(alt)` defects removed; renderer reads the stored payload; bounded substitution reason registered in the audit allowlist; saved-plan changes route through A9. 25 deliberate-breakage mutations, all caught. **Next: A12** (proposal ledger, cooldown persistence, transports), which inherits the binding handoff contract in the architecture plan. |
...
| Approved work outstanding | **Track A**: **A8, A12, A13** — the only outstanding Track A items. Done and merged with reachability proven: A1–A7, A9, A10, A11a, A11b. **W1-44 is closed by A9**; **W1-10 is split into A2 + A8**. **Track B** (WAVE-1 survivors, unchanged): W1-11, W1-13, W1-14, W1-15, W1-16, W1-18, W1-20, W1-21, W1-22, W1-23. |
| A13 (new, owned) | **Persist the load decision to audit.** Surfaced by A5: `LoadRecommendation.to_audit_dict()` is complete but its only consumer is a Mini App debug endpoint, so no past recommendation can be reconstructed. Owner: Observability. Scope is recording-only — no load-behaviour change. Structured fields only, never the Hebrew explanation or raw set rows. Must handle duplicate suppression: `ui.py` calls `recommend_load` per exercise in a render loop. Not blocking A11a. |
