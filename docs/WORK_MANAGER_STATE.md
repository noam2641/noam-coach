...
| Active task | **A10 COMPLETE** (PR #74, `4627e05`, merge `d0e63be`; `git merge-base --is-ancestor 4627e05 origin/develop` exit 0). `build_weekly_plan` now routes through the canonical candidate pipeline: it creates exercise-bearing `plan_versions` candidates and does NOT activate. Confirmation reuses the existing `planv2:select:` tap and activates exactly once through A9. The direct governed-fact write is retired and `noam_coach/bot/onboarding.py` is removed from `_ALLOWED_FACT_WRITER_FILES` — exactly `planning.py` remains authorized. Disclosure covers all four user-facing surfaces, including the candidate choice screen (found by reachability trace, not by a failing test). 15 deliberate-breakage mutations, all caught. **Next: A11b** (serial after A10 per §3.7), which inherits a `build_weekly_plan` returning a real stored plan with concrete exercises — the precondition its renderer rewrite was waiting on. |
