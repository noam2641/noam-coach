...
| Active task | **A10 (degraded-safety readiness) — validated, merge pending.** Branch `feat/a10-degraded-plan`, worktree `wt-a10`. Missing `training_limitations` is `degraded_safety`, not a hard block: the unknown is preserved (`SAFETY_UNKNOWN`), a conservative plan is built, the missing adaptation is disclosed, and activation requires explicit confirmation through A9's `plan_mutations` boundary. **Implementation found a fifth enforcement point** the classification had missed — `training_limitations` is required by the `"workout"` profile as well as `"safety"`, so the workout gate refused before the safety gate ran and the degraded path was unreachable while appearing implemented. Merge order for wave 2 is unchanged: A10 → A11b → A12 (§3.7). |
