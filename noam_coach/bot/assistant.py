...
async def _handle_daily_health_text_action(ctx: FreeTextContext) -> bool:
    ...
    if ctx.action == "pain_resolved":
        # FIX 48: the missing "pain is gone" transition. Resolves the
        # matching active constraint(s) so every reader that already treats
        # status != 'active' as authoritative (training_intelligence.py's
        # active_pain_regions, active_constraints callers) sees it cleared
        # immediately, instead of waiting for a 14-day TTL to silently drop
        # it in some readers while others keep it active forever.
        location = str(ctx.slots.get("location") or "").strip() or None
        resolved_count = await resolve_medical_constraints(
            ctx.user_id, location=location, kind="pain"
        )
        if resolved_count == 0:
            await ctx.send(
                "לא מצאתי דיווח כאב פעיל להסיר — אם זה טעות, ספר לי איפה כאב קודם."
            )
            return True
        await ctx.send("מעולה, רשמתי שהכאב חלף. אחזיר תרגילים רגילים לאזור הזה 💪")
        return True
    return False
...
async def log_meal_from_text(
    ...
    try:
        ...
        await safe_message_edit(progress, friendly_error(exc, "text meal log"))
