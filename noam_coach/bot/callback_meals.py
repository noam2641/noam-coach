...
async def _handle_meal_clarification_actions(
    ...
    if data.startswith("clarify:"):
        ...
        if item is None:
            await query.answer("לא ניתן לקשר את ההבהרה לפריט הנכון", show_alert=True)
            return True
...
async def _handle_meal_decision_actions(
    ...
    if data.startswith("force_approve_meal:"):
        ...
        await clear_meal_fix(user_id)
        if force_analysis:
            totals = force_analysis.totals()
            confirmation = (
                f"נשמר ✅ <b>{esc(force_analysis.meal_name)}</b> — "
                f"{totals['calories']:.0f} קל׳, {totals['protein']:.0f} ג׳ חלבון.\n\n"
                + await build_daily_status(user_id)
            )
        else:
            confirmation = "<b>הארוחה נשמרה ✅</b>\n\n" + await build_daily_status(user_id)
        await safe_edit(
            query,
            confirmation,
            InlineKeyboardMarkup([
                [
                    button("↩️ ביטול שמירה", f"undo_meal:{meal_id}"),
                    button("✏️ תיקון", f"editmeal:{meal_id}"),
                ],
    ...
    if data.startswith("approve_meal:"):
        ...
        if analysis:
            ...
            confirmation = (
                f"נשמר ✅ <b>{esc(analysis.meal_name)}</b> — "
                f"{totals['calories']:.0f} קל׳, {totals['protein']:.0f} ג׳ חלבון.\n\n"
                + await build_daily_status(user_id)
            )
        else:
    ...
    if data.startswith("undo_meal:"):
        meal_id_text = data.split(":", 1)[1]
        if not meal_id_text.isdigit():
            await query.answer("הארוחה כבר אינה זמינה לביטול", show_alert=False)
            return True
